...
const routes = function (config) {
  ...
  exp.importCollection = function (req, res) {
    ...
    if (files.some((file) => !file.data || !file.data.toString)) {
      return res.status(400).send('Bad file data');
    }
    ...
    for (const file of files) {
      ...
      try{
        ...
        if (true === Array.isArray(jsonObject)) {
          jsonObject.forEach((doc) => docs = docs.concat(doc));
        }

        if (false === Array.isArray(jsonObject)) {
          docs = docs.concat(jsonObject)
        }
      } catch(e) {
